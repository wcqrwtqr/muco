...
class jobsdb(models.Model):
    ...
    service = models.CharField(
        max_length=50, default="Eco meter", null=True, blank=True
    )
...
class equipment_job_activitiesdb(models.Model):
    ...
    def get_next(self):
        next = (
            equipment_job_activitiesdb.objects.filter(pk__gt=self.pk)
            .order_by("pk")
            .first()
        )
        if next:
    ...
    def get_prev(self):
        next = (
            equipment_job_activitiesdb.objects.filter(pk__lt=self.pk)
            .order_by("pk")
            .last()
        )
        if next:
